...
const BRAND_NAMES = {
  ...
  ap: "Audemars Piguet", iwc: "IWC", jlc: "Jaeger-LeCoultre", tudor: "Tudor"
};

function getBrandFromSlug(slug) {
  const prefix = slug.split("_")[0];
  return BRAND_NAMES[prefix] || prefix;
}
...
async function generateWatchPage(brand, model, reference) {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "user", content: `Genera una página para el reloj ${brand} ${model} ref ${reference}. JSON exacto: {"title":"nombre completo","description":"2-3 frases atractivas","history":"3-4 frases de historia y contexto","specs":{"diametro":"XX mm","movimiento":"calibre","reserva_marcha":"XX h","resistencia_agua":"XX m","cristal":"tipo","brazalete":"tipo"}}` }]
    })
  });
...
function WatchCard({ watch, onClick }) {
  const brand = getBrandFromSlug(watch.slug || "");
  const colors = { rolex: "#006039", omega: "#c8a84b", patek: "#1a3a6b", ap: "#1a1a1a", iwc: "#8b0000", jlc: "#2c4a2e", tudor: "#8b0000" };
  const prefix = (watch.slug || "").split("_")[0];
  const bg = colors[prefix] || "#1a1a1a";
  return (
    <div style={{ ...S.card, cursor: "pointer", marginBottom: 0 }} onClick={onClick}>
      <div style={{ height: 100, background: bg, borderRadius: 4, marginBottom: 14, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 4 }}>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: "rgba(255,255,255,0.6)", letterSpacing: 2, textTransform: "uppercase" }}>{brand}</div>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 16, color: "#fff", fontWeight: 700 }}>{watch.model}</div>
      </div>
      <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 4 }}>{watch.model}</div>
      <div style={{ ...S.mono, fontSize: 12, color: "#888", marginBottom: 8 }}>Ref. {watch.reference}</div>
      <div style={{ fontSize: 11, color: "#999", fontFamily: "'DM Mono', monospace" }}>@{watch.slug}</div>
    </div>
  );
}
...
function FeedPage({ user, onNavigate }) {
  ...
  async function loadPosts() {
    ...
    if (tab === "following") {
      ...
      const { data } = await supabase.from("posts").select("*, author:profiles(id,name,handle,account_type), watch:watches(id,slug,model,reference)").in("author_id", ids).order("created_at", { ascending: false }).limit(30);
      setPosts(data || []);
    } else {
      const { data } = await supabase.from("posts").select("*, author:profiles(id,name,handle,account_type), watch:watches(id,slug,model,reference)").order("created_at", { ascending: false }).limit(30);
      setPosts(data || []);
    }
    setLoading(false);
  }
...
function ForosPage({ currentUser, onNavigate }) {
  ...
  const [newForm, setNewForm] = useState({ watchQuery: "", watchId: null, watchSlug: "", watchName: "", title: "", content: "" });
  ...
  async function loadRecent() {
    ...
    const { data } = await supabase.from("forum_threads")
      .select("*, author:profiles(id,name,handle), watch:watches(id,slug,model,reference)")
  ...
  async function searchWatches(q) {
    ...
    const { data } = await supabase.from("watches").select("id,slug,model,reference")
      .or(`slug.ilike.%${clean}%,model.ilike.%${clean}%`).limit(6);
    setSuggestions(data || []);
  }

  async function searchForNewThread(q) {
  ...
  async function submitThread() {
    if (!newForm.watchId || !newForm.title.trim() || !newForm.content.trim()) return;
    setPosting(true);
    await supabase.from("forum_threads").insert({ watch_id: newForm.watchId, author_id: currentUser.id, title: newForm.title.trim(), content: newForm.content.trim(), is_news: false });
    setNewForm({ watchQuery: "", watchId: null, watchSlug: "", watchName: "", title: "", content: "" });
  ...
  return (
    <div>
      <div style={{ ...S.row, justifyContent: "space-between", marginBottom: 20 }}>
        <div>
          <h2 style={{ ...S.h1, marginBottom: 4 }}>Foros</h2>
          <p style={S.muted}>Busca por @slug o crea un nuevo hilo</p>
        </div>
      ...
      <div style={{ position: "relative", marginBottom: 24 }}>
        <input style={{ ...S.input, paddingLeft: 36 }} placeholder="Busca @rolex_submariner o escribe un modelo..." value={search}
          onChange={e => { setSearch(e.target.value); searchWatches(e.target.value); }} />
        <span style={{ position: "absolute", left: 12, top: 10, color: "#888", fontSize: 14 }}>🔍</span>
        {suggestions.length > 0 && (
          <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "#fff", border: "1px solid #e8e8e8", borderRadius: 4, boxShadow: "0 4px 12px rgba(0,0,0,0.08)", zIndex: 50 }}>
            {suggestions.map(w => (
              <div key={w.id} style={{ padding: "10px 14px", cursor: "pointer", borderBottom: "1px solid #f5f5f5" }}
                onMouseDown={() => { setSearch(""); setSuggestions([]); onNavigate("watch", w.slug); }}>
                <span style={{ fontWeight: 600 }}>{w.model}</span>
                <span style={{ ...S.mono, color: "#888", fontSize: 12, marginLeft: 8 }}>@{w.slug}</span>
              </div>
            ))}
          </div>
        )}
      ...
      {showNew && (
        <div style={{ ...S.card, border: "1px solid #1a1a1a", marginBottom: 24 }}>
          ...
          <div style={{ marginBottom: 12, position: "relative" }}>
            ...
            <input style={S.input} placeholder="Escribe @rolex_submariner o nombre del modelo..."
              value={newForm.watchQuery}
              onChange={e => { setNewForm(f => ({ ...f, watchQuery: e.target.value, watchId: null, watchSlug: "", watchName: "" })); searchForNewThread(e.target.value); }} />
            {newForm.watchId && <div style={{ marginTop: 6, fontSize: 12, color: "#2a7a4a", fontFamily: "'DM Mono', monospace" }}>✓ @{newForm.watchSlug}</div>}
            {watchSuggestions.length > 0 && !newForm.watchId && (
              <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "#fff", border: "1px solid #e8e8e8", borderRadius: 4, boxShadow: "0 4px 12px rgba(0,0,0,0.08)", zIndex: 50, marginTop: 2 }}>
                {watchSuggestions.map(w => (
                  <div key={w.id} style={{ padding: "10px 14px", cursor: "pointer", borderBottom: "1px solid #f5f5f5" }}
                    onMouseDown={() => { setNewForm(f => ({ ...f, watchQuery: `@${w.slug}`, watchId: w.id, watchSlug: w.slug, watchName: w.model })); setWatchSuggestions([]); }}>
                    <span style={{ fontWeight: 600 }}>{w.model}</span>
                    <span style={{ ...S.mono, color: "#888", fontSize: 12, marginLeft: 8 }}>@{w.slug}</span>
                  </div>
                ))}
          ...
          <div style={{ marginBottom: 12 }}>
            <span style={S.label}>Título del foro</span>
            ...
            {newForm.watchSlug && newForm.title && (
              <div style={{ marginTop: 6, fontSize: 11, color: "#888", fontFamily: "'DM Mono', monospace" }}>
                @{newForm.watchSlug}/{newForm.title.toLowerCase().replace(/\s+/g, "_").replace(/[^a-z0-9_]/g, "").slice(0, 30)}
              </div>
            )}
          </div>
          <div style={{ marginBottom: 16 }}>
            <span style={S.label}>Contenido</span>
            <textarea rows={4} style={{ ...S.input, resize: "none" }} placeholder="Describe el tema del foro..." value={newForm.content} onChange={e => setNewForm(f => ({ ...f, content: e.target.value }))} />
          </div>
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
            <button style={S.btn("outline")} onClick={() => setShowNew(false)}>Cancelar</button>
            <button style={S.btn("primary")} onClick={submitThread} disabled={posting || !newForm.watchId || !newForm.title.trim() || !newForm.content.trim()}>
              {posting ? "Publicando…" : "Crear foro"}
            </button>
          </div>
        </div>
      )}
        ...
        <div key={t.id} style={{ ...S.card, cursor: "pointer" }} onClick={() => onNavigate("thread", t.id)}>
          <div style={{ display: "flex", gap: 14 }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, minWidth: 36 }}>
              <span style={{ fontWeight: 700, fontSize: 15, fontFamily: "'DM Mono', monospace", color: t.votes > 0 ? "#2a7a4a" : t.votes < 0 ? "#d44" : "#888" }}>{t.votes}</span>
              <span style={{ fontSize: 10, color: "#aaa" }}>pts</span>
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ ...S.mono, fontSize: 11, color: "#888", marginBottom: 4, cursor: "pointer" }}
                onClick={e => { e.stopPropagation(); onNavigate("watch", t.watch?.slug); }}>
                @{t.watch?.slug}
              </div>
      ...
      {!loading && recentThreads.length === 0 && <div style={{ ...S.card, textAlign: "center", color: "#888", padding: 32 }}>Aún no hay foros. ¡Crea el primero!</div>}
    </div>
  );
}
...
function WatchPage({ slug, currentUser, onNavigate }) {
  ...
  async function load() {
    ...
    setWatch(w);
    if (w) {
      const { data: p } = await supabase.from("watch_pages").select("*").eq("watch_id", w.id).maybeSingle();
      setPage(p);
      const { data: t } = await supabase.from("forum_threads")
        .select("*, author:profiles(id,name,handle)")
        .eq("watch_id", w.id).order("votes", { ascending: false }).limit(10);
      setThreads(t || []);
    }
    setLoading(false);
  }
  ...
  async function generatePage() {
    ...
    const brand = getBrandFromSlug(watch.slug || "");
    const result = await generateWatchPage(brand, watch.model, watch.reference);
    if (result) {
      const { data: ex } = await supabase.from("watch_pages").select("id").eq("watch_id", watch.id).maybeSingle();
      if (ex) await supabase.from("watch_pages").update({ ...result, updated_at: new Date().toISOString() }).eq("watch_id", watch.id);
      else await supabase.from("watch_pages").insert({ watch_id: watch.id, ...result });
      const { data: p } = await supabase.from("watch_pages").select("*").eq("watch_id", watch.id).single();
      setPage(p);
    }
    setGenerating(false);
  }
  ...
  const colors = { rolex: "#006039", omega: "#c8a84b", patek: "#1a3a6b", ap: "#1a1a1a", iwc: "#8b0000", jlc: "#2c4a2e", tudor: "#8b0000" };
  const prefix = (watch.slug || "").split("_")[0];
  const brandColor = colors[prefix] || "#1a1a1a";

  return (
    <div>
      {/* Hero */}
      <div style={{ height: 140, background: brandColor, borderRadius: 6, marginBottom: 0, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 6 }}>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 12, color: "rgba(255,255,255,0.6)", letterSpacing: 3, textTransform: "uppercase" }}>{brand}</div>
      ...
      <div style={{ ...S.card, borderTopLeftRadius: 0, borderTopRightRadius: 0, borderTop: "none", marginBottom: 20 }}>
        <div style={{ ...S.mono, fontSize: 13, color: "#888" }}>Ref. {watch.reference}{watch.year ? ` · ${watch.year}` : ""}</div>
      </div>
        ...
        <div>
          {page ? (
            <div style={S.card}>
              ...
              <p style={{ fontSize: 14, lineHeight: 1.65, color: "#555", marginBottom: 16 }}>{page.history}</p>
              {page.specs && (
              ...
              <div style={{ marginTop: 12, fontSize: 11, color: "#bbb", fontFamily: "'DM Mono', monospace" }}>Generado por IA</div>
            </div>
          ) : (
            <div style={{ ...S.card, textAlign: "center", padding: 32 }}>
              <p style={S.muted}>Aún no hay ficha para este reloj.</p>
      ...
      {tab === "foros" && (
        <div>
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 16 }}>
            <button style={S.btn("primary")} onClick={() => onNavigate("foros")}>+ Nuevo foro sobre este reloj</button>
          </div>
          {threads.length === 0 && <div style={{ ...S.card, textAlign: "center", color: "#888", padding: 32 }}>Aún no hay foros sobre este reloj.</div>}
          {threads.map(t => (
            <div key={t.id} style={{ ...S.card, cursor: "pointer" }} onClick={() => onNavigate("thread", t.id)}>
              <div style={{ display: "flex", gap: 14 }}>
...
function ThreadPage({ threadId, currentUser, onNavigate }) {
  ...
  async function load() {
    setLoading(true);
    const { data: t } = await supabase.from("forum_threads").select("*, author:profiles(id,name,handle,account_type), watch:watches(id,slug,model)").eq("id", threadId).single();
    setThread(t);
    const { data: r } = await supabase.from("forum_replies").select("*, author:profiles(id,name,handle,account_type)").eq("thread_id", threadId).order("votes", { ascending: false });
  ...
  return (
    <div>
      ...
      <div style={S.card}>
        <div style={{ ...S.mono, fontSize: 11, color: "#888", marginBottom: 8, cursor: "pointer" }} onClick={() => onNavigate("watch", thread.watch?.slug)}>
          @{thread.watch?.slug} / {thread.title.toLowerCase().replace(/\s+/g, "_").slice(0, 30)}
        </div>
        ...
        <div style={S.row}>
          <button style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18, color: "#888", padding: "0 4px" }} onClick={() => voteThread(1)}>▲</button>
...
function RelojesPage({ onNavigate }) {
  ...
  async function load() {
    setLoading(true);
    const { data } = await supabase.from("watches").select("*").order("model");
    setWatches(data || []); setLoading(false);
  }

  const filtered = watches.filter(w =>
    w.model.toLowerCase().includes(search.toLowerCase()) ||
    (w.slug || "").toLowerCase().includes(search.toLowerCase()) ||
    getBrandFromSlug(w.slug || "").toLowerCase().includes(search.toLowerCase())
  );

  const byBrand = filtered.reduce((acc, w) => {
    const brand = getBrandFromSlug(w.slug || "");
    ...
    <div>
      ...
      {loading ? <Spinner /> : Object.entries(byBrand).map(([brand, ws]) => (
        <div key={brand} style={{ marginBottom: 32 }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, fontFamily: "'DM Mono', monospace", letterSpacing: 1, textTransform: "uppercase", color: "#888", marginBottom: 14 }}>{brand}</h3>
...
function ProfilePage({ userId, currentUser, onNavigate }) {
  ...
  async function load() {
    ...
    const [{ data: p }, { data: po }, { data: w }, { data: f }] = await Promise.all([
      ...
      supabase.from("posts").select("*, author:profiles(id,name,handle,account_type), watch:watches(id,slug,model,reference)").eq("author_id", userId).order("created_at", { ascending: false }),
  ...
  async function addWatch(watch) {
    setAdding(true);
    const { data: ex } = await supabase.from("watch_registrations").select("id").eq("user_id", currentUser.id).eq("watch_id", watch.id).maybeSingle();
    if (!ex) await supabase.from("watch_registrations").insert({ user_id: currentUser.id, watch_id: watch.id, is_public: true });
    setWatchSearch(""); setWatchSuggestions([]); setShowAddWatch(false); await load(); setAdding(false);
  }
    ...
    <div>
      <div style={S.card}>
        ...
        <div style={{ display: "flex", gap: 28, marginBottom: 16 }}>
          ...
          <div><div style={{ fontWeight: 700, fontSize: 18 }}>{watches.length}</div><div style={S.muted}>relojes</div></div>
        </div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 20 }}>
        ...
        <button style={S.navLink(tab === "coleccion")} onClick={() => setTab("coleccion")}>Colección ({watches.length})</button>
      </div>
        ...
        <div>
          {isOwn && (
            <div style={{ marginBottom: 20 }}>
              {!showAddWatch ? (
                <button style={S.btn("outline")} onClick={() => setShowAddWatch(true)}>+ Añadir reloj a mi colección</button>
              ) : (
                <div style={{ ...S.card, border: "1px solid #1a1a1a", position: "relative" }}>
                  <span style={S.label}>Busca tu reloj</span>
                  <input style={S.input} placeholder="ej: Submariner, @rolex_daytona..." value={watchSearch}
                    onChange={e => { setWatchSearch(e.target.value); searchWatches(e.target.value); }} autoFocus />
                  {watchSuggestions.length > 0 && (
                    <div style={{ background: "#fff", border: "1px solid #e8e8e8", borderRadius: 4, marginTop: 4, boxShadow: "0 4px 12px rgba(0,0,0,0.08)" }}>
                      {watchSuggestions.map(w => (
                        <div key={w.id} style={{ padding: "10px 14px", cursor: "pointer", borderBottom: "1px solid #f5f5f5", display: "flex", justifyContent: "space-between", alignItems: "center" }}
                  ...
                  <div style={{ marginTop: 10 }}>
                    <button style={S.btn("outline")} onClick={() => { setShowAddWatch(false); setWatchSearch(""); setWatchSuggestions([]); }}>Cancelar</button>
                  </div>
                </div>
              )}
            </div>
          ...
          {watches.length === 0 && <p style={S.muted}>Colección vacía o privada.</p>}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
            {watches.map(w => w.watch && (
              <WatchCard key={w.id} watch={w.watch} onClick={() => onNavigate("watch", w.watch.slug)} />
            ))}
          </div>
        </div>
      )}
...
// ─── MY COLLECTION ────────────────────────────────────────────────────────────
function MyCollectionPage({ user, onNavigate }) {
  ...
  return (
    <div>
      ...
      <p style={{ ...S.muted, marginBottom: 20 }}>Gestiona tus relojes desde tu perfil</p>
      <button style={S.btn("primary")} onClick={() => onNavigate("profile", user.id)}>Ir a mi perfil → Colección</button>
  ...
  return (
    <div style={S.app}>
      ...
      <main style={S.main}>
        ...
        {page.name === "collection" && <MyCollectionPage user={session.user} onNavigate={navigate} />}
      </main>
    </div>
  );
}
