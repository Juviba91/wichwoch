    ...
    <div>
      ...
      {tab==="relojes" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:14 }}>
          ...
          {watches.length===0 && <p style={S.muted}>Sin relojes registrados aún.</p>}
        </div>
      )}
        ...
        <div>
          {isOwner && (
            <div style={{ marginBottom:16 }}>
              {!showNewsForm ? (
                <button style={S.btn("primary")} onClick={()=>setShowNewsForm(true)}>+ Publicar novedad</button>
              ) : (
                <div style={{ ...S.card, border:"1px solid #1a1a1a" }}>
                  ...
                  <div style={{ ...S.row, justifyContent:"space-between" }}>
                    <label style={{ ...S.row, gap:8, fontSize:13, cursor:"pointer" }}>
                      <input type="checkbox" checked={newsForm.owners_only} onChange={e=>setNewsForm(f=>({...f,owners_only:e.target.checked}))} />
                      Solo propietarios verificados
                    </label>
                    <div style={S.row}>
                      <button style={S.btn("outline")} onClick={()=>setShowNewsForm(false)}>Cancelar</button>
                      <button style={S.btn("primary")} onClick={postNews} disabled={posting}>{posting?"Publicando…":"Publicar"}</button>
                    </div>
                  </div>
                </div>
              )}
          ...
          {news.length===0 && <div style={{ ...S.card, textAlign:"center", color:"#888", padding:32 }}>Sin novedades publicadas aún.</div>}
          {news.map(n=>(
            <div key={n.id} style={S.card}>
              <div style={{ ...S.row, justifyContent:"space-between", marginBottom:8 }}>
                <div style={{ fontWeight:700, fontSize:15 }}>{n.title}</div>
                <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                  {n.owners_only && <Badge text="Solo propietarios" bg="#fff8e8" color="#b8860b" />}
...
function WatchPage({ slug, currentUser, onNavigate }) {
  ...
  return (
    <div>
      <div style={{ height:130, background:bg, borderRadius:6, display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:6, marginBottom:0 }}>
        <div style={{ fontFamily:"'DM Mono',monospace", fontSize:11, color:"rgba(255,255,255,0.55)", letterSpacing:3, textTransform:"uppercase", cursor:"pointer" }} onClick={()=>onNavigate("brand",watch.brand_slug)}>
          @{watch.brand_slug} →
        </div>
      ...
      {tab==="foros" && (
        <div>
          <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:16 }}>
            <button style={S.btn("primary")} onClick={()=>onNavigate("foros")}>+ Crear foro sobre este reloj</button>
          </div>
          {threads.length===0 && <div style={{ ...S.card, textAlign:"center", color:"#888", padding:32 }}>Aún no hay foros sobre este reloj. ¡Crea el primero!</div>}
          {threads.map(t=>(
...
function ForosPage({ currentUser, onNavigate }) {
  ...
  async function loadRecent() {
    ...
    const {data} = await supabase.from("forum_threads")
      .select("*, author:profiles(id,name,handle), watch:watches(id,slug,model)")
  ...
  async function submitThread() {
    setPostError(null);
    if(!newForm.watchId) { setPostError("Selecciona un reloj de la lista."); return; }
    ...
    <div>
      <div style={{ ...S.row, justifyContent:"space-between", marginBottom:20 }}>
        <div>
          ...
          <p style={S.muted}>Debates sobre relojes concretos</p>
        </div>
        ...
        <div style={{ ...S.card, border:"1px solid #1a1a1a", marginBottom:24 }}>
          ...
          <div style={{ marginBottom:12, position:"relative" }}>
            ...
            <input style={S.input} placeholder="Escribe @rolex_submariner o nombre del modelo..."
              ...
              onChange={e=>{ setNewForm(f=>({...f,watchQuery:e.target.value,watchId:null,watchSlug:""})); searchForNew(e.target.value); }} />
            {newForm.watchId && <div style={{ marginTop:5, fontSize:12, color:"#2a7a4a", fontFamily:"'DM Mono',monospace" }}>✓ @{newForm.watchSlug} seleccionado</div>}
            {watchSuggestions.length>0 && !newForm.watchId && (
              <div style={{ position:"absolute", top:"100%", left:0, right:0, background:"#fff", border:"1px solid #e8e8e8", borderRadius:4, boxShadow:"0 4px 12px rgba(0,0,0,0.08)", zIndex:50, marginTop:2 }}>
                {watchSuggestions.map(w=>(
                  <div key={w.id} style={{ padding:"10px 14px", cursor:"pointer", borderBottom:"1px solid #f5f5f5" }}
                    onMouseDown={()=>{ setNewForm(f=>({...f,watchQuery:`@${w.slug}`,watchId:w.id,watchSlug:w.slug})); setWatchSuggestions([]); }}>
                    <span style={{ fontWeight:600 }}>{w.model}</span>
                    <span style={{ ...S.mono, color:"#888", fontSize:12, marginLeft:8 }}>@{w.slug}</span>
                  </div>
                ))}
          ...
          <div style={{ marginBottom:12 }}>
            ...
            {newForm.watchSlug && newForm.title && (
              <div style={{ marginTop:5, fontSize:11, color:"#aaa", fontFamily:"'DM Mono',monospace" }}>
      ...
      {loading ? <Spinner /> : recentThreads.map(t=>(
        <div key={t.id} style={{ ...S.card, cursor:"pointer" }} onClick={()=>onNavigate("thread",t.id)}>
          <div style={{ display:"flex", gap:14 }}>
            ...
            <div style={{ flex:1 }}>
              <div style={{ ...S.mono, fontSize:11, color:"#888", marginBottom:3, cursor:"pointer" }} onMouseDown={e=>{ e.stopPropagation(); onNavigate("watch",t.watch?.slug); }}>@{t.watch?.slug}</div>
              <div style={{ fontWeight:700, fontSize:15, marginBottom:4 }}>{t.title}</div>
              <div style={{ display:"flex", gap:16 }}>
                <span style={S.muted}>@{t.author?.handle}</span>
                <span style={S.muted}>💬 {t.replies_count||0}</span>
...
function ProfilePage({ userId, currentUser, onNavigate }) {
  ...
  async function addWatch(watch) {
    ...
    setWatchSearch(""); setWatchSuggestions([]); setShowAddWatch(false); await load();
  }
        ...
        <div>
          {isOwn && (
            <div style={{ marginBottom:20 }}>
              {!showAddWatch ? (
                <button style={S.btn("outline")} onClick={()=>setShowAddWatch(true)}>+ Añadir reloj</button>
              ) : (
                <div style={{ ...S.card, border:"1px solid #1a1a1a" }}>
